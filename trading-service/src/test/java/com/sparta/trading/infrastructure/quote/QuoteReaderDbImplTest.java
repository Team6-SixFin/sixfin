...
class QuoteReaderDbImplTest {
    ...
    private MarketClockSnapshot clockOf(ClockStatus status) {
    ...
    private void givenCurrentSeq(MarketClockSnapshot marketClock, long seq) {
    ...
    void read_returnsQuoteMappedFromCandle() {
        ...
        MarketClockSnapshot marketClock = clockOf(ClockStatus.RUNNING);
    ...
    void readAll_propagatesMarketClockNotFound() {
        when(stocksRepository.findIdBySymbolIn(List.of("AAPL"))).thenReturn(List.of(1L));
        when(currentSeqProvider.getClockSnapshot())
