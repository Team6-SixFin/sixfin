...
public class OrderExecutionService {
    ...
    /**
     * 계좌 비관적 잠금부터 주문·체결·포지션·원장·Outbox 저장까지를 하나의 트랜잭션으로 처리한다.
     */
    @Transactional
    public OrderExecutionResult execute(UUID userId, NormalizedOrder normalized, Quote quote) {
        // 계좌 행을 비관적 락으로 조회
        ...
        Optional<Orders> existingOrder = ordersQueryRepository.findByAccountIdAndRequestId(
                account.getId(), normalized.requestId()
        );
        if (existingOrder.isPresent()) {
            return existingResult(existingOrder.get(), normalized, quote.stockId(), account);
        }
     ...
     */
    private OrderExecutionResult executeBuy(
            UUID userId,
        ...
        if (isNewPosition) {
            position = positionsCommandRepository.save(Positions.open(
                    ...
                    normalized.quantity(),
                    quote.price(),
                    normalized.plannedStopLossPrice(),
                    normalized.investmentReason(),
                    quote.marketTime(),
                    quote.seq(),
                    userId
            ));
        ...
        return new OrderExecutionResult(order, execution, cashBalanceAfter);
    }
     ...
     */
    private OrderExecutionResult executeSell(
            UUID userId,
            NormalizedOrder normalized,
            Quote quote,
            Accounts account
    ...
    /** 주문을 거절 상태로 저장하고, 거절된 주문 정보를 결과로 반환한다. */
    private OrderExecutionResult reject(
            Accounts account,
            NormalizedOrder normalized,
            Quote quote,
        ...
        return new OrderExecutionResult(order, null, account.getCashBalance());
    }

    /** 기존 주문 처리 (검증 및 결과 반환) */
    private OrderExecutionResult existingResult(
            Orders order, // 기존 주문
            NormalizedOrder normalized,
            Long stockId,
        ...
        return new OrderExecutionResult(order, execution, account.getCashBalance());
    }
    ...
    private TradingEventEnvelope toPositionClosedEvent(
            UUID eventId,
            UUID userId,
            Positions position,
            Quote quote
